...
public class AppointmentManager
{
  ...
  public Payment CompleteAppointment(int appointmentId, decimal amountPaid, string method, Employee emp, Service svc)
    ...
    public void CancelAppointment(int appointmentId)
    {
        ...
        if (ap.Status == AppointmentStatus.Completed)
        {
            throw new InvalidOperationException("This appointment cannot be cancelled as it has already been completed.");
        }
    ...
    public bool UpdateAppointment(Appointment updatedAp)
    {
        var conflict = _db.Appointments
            .Where(a =>
                a.Id != updatedAp.Id && 
                a.Status == AppointmentStatus.Scheduled &&
                (a.EmployeeId == updatedAp.EmployeeId || a.ClientId == updatedAp.ClientId)
            )
            .AsEnumerable()
            .Any(a => !(updatedAp.EndAt <= a.StartAt || updatedAp.StartAt >= a.EndAt));

        if (conflict) return false;

        _db.Attach(updatedAp).State = EntityState.Modified;
        _db.SaveChanges();
